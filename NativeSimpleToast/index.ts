import { toastOptionInterface } from "./type";

function createSVGElement(type:'info'|'warn'|'success'|'error'){
...
const defaultOption:toastOptionInterface = {
    icon: 'info',
    message: '',
    duration: 50000,
}
...
interface Toast {
    useOption: toastOptionInterface,
    icon: 'info' | 'warn' | 'error' | 'success',
    message: string,
    duration: number,
    element: null | HTMLDivElement,
    box: null | HTMLDivElement
}

class Toast {
    constructor(option:toastOptionInterface){
        this.useOption = {...defaultOption,...option};
    ...
    boxCreateInitialize(){
        this.box = <null|HTMLDivElement>document.getElementById('ns-toast-box');
        if(this.box === null){
    ...
    createToast(){
        ...
        this.box?.appendChild(toast_div);
        this.element = toast_div;
        setTimeout(() => {
            // (<HTMLDivElement>this.element).style.opacity = '1';
            toast_div.style.transform = 'translateX(0)';
    }, 100);
    }

    removeToast(){
        // (<HTMLDivElement>this.element).style.opacity = '0';
        (<HTMLDivElement>this.element).style.transform = 'translateX(150%)';
        console.log('remove-toast');
        setTimeout(() => {
            this.box?.removeChild(<HTMLDivElement>this.element);
            const toasts = this.box?.getElementsByTagName('div');
            if(toasts?.length === 0){
                document.body.removeChild(<HTMLDivElement>this.box);
            }
        }, 1000);
